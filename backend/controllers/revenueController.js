...
export const getRevenueBreakdown = expressAsyncHandler(async (req, res) => {
  ...
  const baseMatch = { createdAt: { $gte: yearStart, $lte: yearEnd } };
  ...
  const daily = Array.from({ length: daysInMonth }, (_, i) => {
    ...
    return { day: i + 1, revenue: d?.revenue ?? 0, count: d?.count ?? 0 };
  });
...
export const getRevenueStats = expressAsyncHandler(async (req, res) => {
  const now         = new Date();
  const thisMonth   = { $gte: startOf(now.getFullYear(), now.getMonth()), $lte: endOf(now.getFullYear(), now.getMonth()) };
  const lastMonthS  = startOf(now.getFullYear(), now.getMonth() - 1);
  const lastMonthE  = endOf(now.getFullYear(),   now.getMonth() - 1);
  const lastMonth   = { $gte: lastMonthS, $lte: lastMonthE };
  const todayS      = new Date(now.getFullYear(), now.getMonth(), now.getDate());
  const todayE      = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 23, 59, 59, 999);
  const today       = { $gte: todayS, $lte: todayE };

  const [thisMonthAgg, lastMonthAgg, todayAgg, pendingAgg] = await Promise.all([
    Bookings.aggregate([
      { $match: { status: { $in: ["confirmed","completed"] }, createdAt: thisMonth } },
      { $group: { _id: null, revenue: { $sum: "$totalAmount" }, count: { $sum: 1 } } },
    ]),
    Bookings.aggregate([
      { $match: { status: { $in: ["confirmed","completed"] }, createdAt: lastMonth } },
      { $group: { _id: null, revenue: { $sum: "$totalAmount" }, count: { $sum: 1 } } },
    ]),
    Bookings.aggregate([
      { $match: { status: { $in: ["confirmed","completed"] }, createdAt: today } },
      { $group: { _id: null, revenue: { $sum: "$totalAmount" }, count: { $sum: 1 } } },
    ]),
    Bookings.aggregate([
  ...
  const thisM  = thisMonthAgg[0]  ?? { revenue: 0, count: 0 };
  const lastM  = lastMonthAgg[0]  ?? { revenue: 0, count: 0 };
  const tod    = todayAgg[0]      ?? { revenue: 0, count: 0 };
  const pend   = pendingAgg[0]    ?? { revenue: 0, count: 0 };

  const momGrowth = lastM.revenue > 0
    ? Number((((thisM.revenue - lastM.revenue) / lastM.revenue) * 100).toFixed(1))
    : null;

  res.status(200).json({
    success: true,
    stats: {
      today:        { revenue: tod.revenue,   count: tod.count   },
      thisMonth:    { revenue: thisM.revenue, count: thisM.count },
      lastMonth:    { revenue: lastM.revenue, count: lastM.count },
      pending:      { revenue: pend.revenue,  count: pend.count  },
      momGrowth,
    },
  });
});