...
const TICKET_POPULATE = [
  ...
  { path: "bookingId",        select: "bookingReference seats seatClass totalAmount status" },
  { path: "flightId",         select: "flightNumber source destination departureTime arrivalTime status" },
...
export const createTicket = expressAsyncHandler(async (req, res) => {
  ...
  if (bookingId) {
    const booking = await Bookings.findOne({ _id: bookingId, passengerId: req.user._id });
    if (!booking) {
      return res.status(400).json({ success: false, message: "Invalid booking — it does not belong to you" });
    }
  }
...
export const getTicketById = expressAsyncHandler(async (req, res) => {
  ...
  if (isAdmin && ticket.unreadByAdmin > 0) {
    await SupportTickets.findByIdAndUpdate(ticket._id, {
      $set: { "messages.$[].readByAdmin": true }, unreadByAdmin: 0,
    });
    ticket.unreadByAdmin = 0;
  }
  if (isOwner && ticket.unreadByUser > 0) {
    await SupportTickets.findByIdAndUpdate(ticket._id, {
      $set: { "messages.$[].readByUser": true }, unreadByUser: 0,
    });
...
export const getAllTickets = expressAsyncHandler(async (req, res) => {
  const { status, priority, category, search, page = 1, limit = 15 } = req.query;
...
export const getTicketsByFlight = expressAsyncHandler(async (req, res) => {
  ...
  if (req.user.role === "crew") {
    ...
    if (!crewDoc) {
      return res.status(403).json({ success: false, message: "Crew profile not found" });
    }
...
export const deleteTicket = expressAsyncHandler(async (req, res) => {
  ...
  res.status(200).json({ success: true, message: `Ticket ${ticket.ticketNumber} deleted` });
});