...
function Badge({ label, cls }) {
  return (
    <span
      className={`text-[11px] font-semibold px-2.5 py-1 rounded-full whitespace-nowrap ${cls}`}
    >
      {label}
    </span>
  );
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 flex flex-col items-center min-w-[80px]">
                ...
                <div className="relative w-full flex items-center">
                  ...
                  <svg
                    className="w-3.5 h-3.5 text-[#0C3060] mx-1 flex-shrink-0"
                    viewBox="0 0 24 24"
                    fill="currentColor"
                  >
        ...
        <div className="border-t border-slate-100 px-5 py-4 bg-[#F8FAFC]">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              {
                label: "Aircraft",
                value: flight.aircraftId?.registrationNumber || "—",
              },
...
export default function DealsPage() {
  ...
  const { data: scheduledData, isLoading: l1 } = useGetAllFlightsQuery({
    status: "scheduled",
    limit: 200,
    page: 1,
  });
  const { data: boardingData, isLoading: l2 } = useGetAllFlightsQuery({
    status: "boarding",
    limit: 200,
    page: 1,
  });
  const { data: delayedData, isLoading: l3 } = useGetAllFlightsQuery({
    status: "delayed",
    limit: 200,
    page: 1,
  });
  ...
  const filtered = useMemo(() => {
    ...
    if (dateFrom) {
      result = result.filter(
        (f) => new Date(f.departureTime) >= new Date(dateFrom),
      );
    }
    if (dateTo) {
      ...
      <div className="bg-[#0C3060] px-6 py-10 relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          ...
          <h1 className="text-white text-3xl font-bold mb-1">
            Deals & upcoming flights
          </h1>
          <p className="text-blue-200/70 text-[14px]">
            {isLoading
              ? "Loading..."
              : `${filtered.length} flight${filtered.length !== 1 ? "s" : ""} available`}
          </p>
        </div>
      </div>
            ...
            <div className="lg:hidden fixed inset-0 z-[100] flex">
              ...
              <div className="relative ml-auto w-80 bg-white h-full overflow-y-auto p-5 shadow-2xl">
                <div className="flex items-center justify-between mb-4">
                  <p className="text-[14px] font-bold text-[#0C3060]">
                    Filters
                  </p>
                  <button
                    onClick={() => setShowFilters(false)}
                    className="text-slate-400 hover:text-[#0C3060] cursor-pointer bg-transparent border-none"
                  >
                    <svg
                      className="w-5 h-5"
                      viewBox="0 0 24 24"
          ...
          <div className="flex-1 min-w-0">
            ...
            {activeFilterCount > 0 && (
              <div className="flex items-center gap-2 flex-wrap mb-4">
                ...
                {selectedStops.map((s) => (
                  <span
                    key={s}
                    className="flex items-center gap-1.5 bg-blue-50 text-blue-700 text-[11px] font-semibold px-2.5 py-1 rounded-full"
                  >
                    {s === "0" ? "Non-stop" : s === "1" ? "1 stop" : "2+ stops"}
                    <button
                      onClick={() =>
                        setSelectedStops((p) => p.filter((x) => x !== s))
                      }
                      className="hover:text-blue-900 cursor-pointer bg-transparent border-none leading-none"
                    >
                      ×
                    </button>
                  </span>
                ...
                {selectedStatus.map((s) => (
                  <span
                    key={s}
                    className="flex items-center gap-1.5 bg-violet-50 text-violet-700 text-[11px] font-semibold px-2.5 py-1 rounded-full"
                  >
                    {STATUS_CONFIG[s]?.label || s}
                    <button
                      onClick={() =>
                        setSelectedStatus((p) => p.filter((x) => x !== s))
                      }
                      className="hover:text-violet-900 cursor-pointer bg-transparent border-none leading-none"
                    >
                      ×
                    </button>
                  </span>
                ...
                {priceRange < maxPrice && (
                  <span className="flex items-center gap-1.5 bg-green-50 text-green-700 text-[11px] font-semibold px-2.5 py-1 rounded-full">
                    ...
                    <button
                      ...
                      className="hover:text-green-900 cursor-pointer bg-transparent border-none leading-none"
                    >
                      ×
                    </button>
                  </span>
                ...
                {(dateFrom || dateTo) && (
                  <span className="flex items-center gap-1.5 bg-orange-50 text-orange-700 text-[11px] font-semibold px-2.5 py-1 rounded-full">
                    ...
                    <button
                      ...
                      className="hover:text-orange-900 cursor-pointer bg-transparent border-none leading-none"
                    >
                      ×
                    </button>
                  </span>
                ...
                <button
                  ...
                  className="text-[11px] text-red-400 font-semibold hover:underline cursor-pointer bg-transparent border-none"
                >
                  Clear all
                </button>
              </div>
            ...
            {isLoading ? (
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-16 text-center">
                ...
                <button
                  onClick={resetFilters}
                  className="mt-4 text-[12px] text-blue-500 font-semibold hover:underline cursor-pointer bg-transparent border-none"
                >
                  Reset all filters
                </button>
              </div>
