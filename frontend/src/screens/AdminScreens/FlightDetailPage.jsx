  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
      {rows.map(([label, value]) => (
        <div
          key={label}
          className="flex justify-between items-center py-3 border-b border-[#F0F7FF]"
        >
          <span className="text-[12px] font-medium text-[#7A90A4]">
            {label}
          </span>
          <span className="text-[12px] font-semibold text-[#0D1B2A] text-right max-w-[55%]">
            {value ?? "—"}
          </span>
        </div>
      ))}
...
function Avatar({ name = "", size = "md" }) {
  ...
  return (
    <div
      className={`${s[size]} rounded-full bg-[#E3F2FD] text-[#1565C0] flex items-center justify-center font-bold flex-shrink-0`}
    >
      {initials || "?"}
    </div>
  );
  ...
  if (isError || !flight) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-3">
        ...
        <p className="text-[15px] font-semibold text-[#7A90A4]">
          Flight not found
        </p>
        <button
          onClick={() => navigate(-1)}
          className="text-[13px] text-[#1565C0] font-semibold hover:underline cursor-pointer bg-transparent border-none"
        >
          ← Go back
        </button>
      </div>
      ...
      <div className="bg-white border-b border-[#D0E6F7] px-5 md:px-8 py-4 flex items-center gap-4 sticky top-0 z-50">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2.5 flex-wrap">
            <h1 className="text-[17px] font-bold text-[#0D1B2A]">
              {flight.flightNumber}
            </h1>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3.5 mb-6">
          {[
            ...
            },
            {
              ...
              value: `${flight.availableSeats} / ${flight.totalSeats}`,
              icon: "ti-armchair",
              color: "bg-green-50 text-green-700",
            },
            {
              label: "Duration",
              value: duration(flight.departureTime, flight.arrivalTime),
              icon: "ti-clock",
            ...
            <div
              key={label}
              className="bg-white border border-[#D0E6F7] rounded-2xl p-5"
            >
              ...
              <p className="text-[22px] font-bold text-[#0D1B2A] leading-none">
                {value}
              </p>
            </div>
          ))}
        ...
        <div className="bg-white border border-[#D0E6F7] rounded-2xl p-5">
          ...
          {tab === "overview" && (
            <>
              ...
              <InfoGrid
                rows={[
                  ...
                  [
                    ...
                    stops === 0
                      ? "Non-stop"
                      : `${stops} intermediate stop${stops > 1 ? "s" : ""}`,
                  ],
                  ...
                  [
                    "Booked Seats",
                    (flight.totalSeats || 0) - (flight.availableSeats || 0),
                  ],
                ]}
              />
              ...
              {flight.routes?.length > 0 && (
                <div className="mt-5">
                  <p className="text-[11px] font-bold text-[#5A7089] uppercase tracking-[0.8px] mb-3">
                    Route stops
                  </p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {flight.crewIds.map((c) => {
                    ...
                    return (
                      <div
                        key={c._id}
                        className="flex items-center gap-4 p-4 bg-[#F7FAFD] border border-[#EAF4FB] rounded-2xl hover:border-[#1565C0] transition"
                      >
                        <Avatar name={name} size="lg" />
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                            <p className="text-[13px] font-bold text-[#0D1B2A] truncate">
                              {name}
                            </p>
                            <span
                          ...
                          <div className="flex items-center gap-1.5 mb-1">
                            <i
                              className={`ti ${roleIcon} text-[#1565C0] text-[12px]`}
                            />
                            <p className="text-[12px] text-[#1565C0] font-semibold">
                              {c.role}
                            </p>
                          </div>
                          {[
                            ...
                            [
                              "Experience",
                              c.experience ? `${c.experience} yrs` : null,
                            ],
                          ].map(([label, value]) =>
                            value ? (
                              <p
                                key={label}
                                className="text-[11px] text-[#7A90A4]"
                              >
                ...
                <div className="mt-5 pt-4 border-t border-[#EAF4FB]">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {Object.entries(
                      ...
                      <span
                        key={role}
                        className="text-[12px] font-semibold bg-[#EAF4FB] text-[#1565C0] px-3 py-1.5 rounded-xl"
                      >
                        {role} × {count}
                      </span>
                    ))}
              ...
              {!flight.aircraftId ? (
                ...
                <>
                  <div className="flex items-center gap-4 bg-[#EAF4FB] rounded-2xl p-5 mb-5">
                    ...
                    <div>
                      <p className="text-[18px] font-black text-[#0D1B2A]">
                        {flight.aircraftId.registrationNumber}
                      </p>
                      <p className="text-[13px] text-[#1565C0] font-semibold">
                        {flight.aircraftId.model}
                      </p>
                      <p className="text-[11px] text-[#7A90A4] mt-0.5">
                        Capacity: {flight.aircraftId.capacity} seats
                      </p>
                    </div>
                  </div>
                  <InfoGrid
            ...
            <>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
                {[
                  ...
                  {
                    label: "Revenue",
                    value: `₹${(bookingStats.revenue ?? 0).toLocaleString()}`,
                    color: "bg-violet-50 text-violet-700",
                    icon: "ti-currency-rupee",
                  },
              ...
              <div className="bg-[#EAF4FB] rounded-2xl p-5 mb-5">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    ...
                    <p className="text-[11px] text-[#7A90A4] mt-0.5">
                      {(flight.totalSeats || 0) - (flight.availableSeats || 0)}{" "}
                      booked of {flight.totalSeats} total
                    </p>
                  </div>
              ...
              {bookingsLoading ? (
                <div className="flex items-center justify-center py-12 gap-3">
                  ...
                  <span className="text-[13px] text-[#7A90A4]">
                    Loading bookings...
                  </span>
                </div>
              ) : bookings.length === 0 ? (
                <div className="text-center py-10 bg-[#F7FAFD] border border-[#EAF4FB] rounded-2xl">
                  ...
                  <p className="text-[11px] text-[#B0C4D8] mt-1">
                    {bookingFilter !== "all"
                      ? `No ${bookingFilter} bookings for this flight`
                      ...
                      <thead>
                        <tr>
                          {[
                            ...
                            <th
                              key={h}
                              className="text-[10px] font-semibold text-[#7A90A4] uppercase tracking-[0.5px] pb-3 text-left pr-4 whitespace-nowrap"
                            >
                              {h}
                            </th>
                          ))}
                      ...
                      <tbody>
                        {bookings.map((b) => (
                          <tr key={b._id}>
                            ...
                            {/* Ref */}
                            <td className="py-3 pr-4 border-t border-[#EAF4FB] align-middle">
                              <span className="text-[11px] font-black text-[#1565C0] tracking-wide">
                                {b.bookingReference}
                              </span>
                            </td>
                            {/* Seats */}
                            <td className="py-3 pr-4 border-t border-[#EAF4FB] align-middle">
                              <div className="flex flex-wrap gap-1">
                                {b.seats?.map((s) => (
                                  <span
                                    key={s}
                                    className="text-[10px] font-bold bg-[#EAF4FB] text-[#1565C0] px-1.5 py-0.5 rounded-md"
                                  >
                                    {s}
                                  </span>
                                ))}
                            ...
                            <td className="py-3 pr-4 border-t border-[#EAF4FB] align-middle">
                              <span
                                className={`text-[11px] font-semibold px-2.5 py-[3px] rounded-full whitespace-nowrap capitalize ${BOOKING_STATUS_CLS[b.status] || "bg-gray-100 text-gray-600"}`}
                              >
                                {b.status}
                              </span>
                            </td>
                            ...
                            <td className="py-3 pr-4 border-t border-[#EAF4FB] align-middle">
                              <span
                                className={`text-[11px] font-semibold px-2.5 py-[3px] rounded-full whitespace-nowrap capitalize ${PAYMENT_STATUS_CLS[b.paymentStatus] || "bg-gray-100 text-gray-600"}`}
                              >
                                {b.paymentStatus}
                              </span>
                            </td>
              ...
              <div className="relative">
                ...
                {[
                  ...
                  },
                  {
                    ...
                    time: fmt(flight.departureTime),
                    icon: "ti-calendar",
                    color: "bg-violet-100 text-violet-700",
                    done: new Date() >= new Date(flight.departureTime),
                  },
                  {
                    label: "Boarding started",
                    time: ["boarding", "in-flight", "completed"].includes(
                      flight.status,
                    )
                      ? "Confirmed"
                      : "Pending",
                    icon: "ti-door-enter",
                    color: ["boarding", "in-flight", "completed"].includes(
                      flight.status,
                    )
                      ? "bg-violet-100 text-violet-700"
                      : "bg-gray-100 text-gray-400",
                    done: ["boarding", "in-flight", "completed"].includes(
                      flight.status,
                    ),
                  },
                  {
                    label: "Departed",
                    time: ["in-flight", "completed"].includes(flight.status)
                      ? "Confirmed"
                      : "Pending",
                    icon: "ti-plane-departure",
                    color: ["in-flight", "completed"].includes(flight.status)
                      ? "bg-blue-100 text-blue-700"
                      : "bg-gray-100 text-gray-400",
                    done: ["in-flight", "completed"].includes(flight.status),
                  },
                  ...(flight.routes?.slice(1, -1).map((stop, i) => ({
                    ...
                    color: "bg-orange-100 text-orange-700",
                      ...
                      flight.currentStop &&
                      flight.routes?.indexOf(flight.currentStop) > i + 1,
                  })) || []),
                  {
                    ...
                    time: flight.actualArrivalTime
                      ...
                      : flight.status === "completed"
                        ? "Confirmed"
                        : fmt(flight.arrivalTime) + " (scheduled)",
                    icon: "ti-plane-arrival",
                    color:
                      flight.status === "completed"
                        ? "bg-green-100 text-green-700"
                        : "bg-gray-100 text-gray-400",
                    done: flight.status === "completed",
                  },
                ].map(({ label, time, icon, color, done }, i) => (
                  <div key={i} className="flex gap-4 mb-5 relative">
                    <div
                      className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 relative z-10 ${done ? color : "bg-gray-100 text-gray-300"}`}
                    >
                      <i className={`ti ${icon} text-[16px]`} />
                    </div>
                    <div className="flex-1 pt-1.5">
                      <p
                        className={`text-[13px] font-bold ${done ? "text-[#0D1B2A]" : "text-[#B0C4D8]"}`}
                      >
                        {label}
                      </p>
                      <p
                        className={`text-[11px] mt-0.5 ${done ? "text-[#7A90A4]" : "text-[#D0E6F7]"}`}
                      >
                        {time}
                      </p>
                    </div>
                    {done && (
                      <i className="ti ti-check text-green-500 text-[14px] pt-2 flex-shrink-0" />
                    )}
                  </div>
                ))}
              </div>
