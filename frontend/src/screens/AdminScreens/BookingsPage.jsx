...
const bookingsApi = apiSlice.injectEndpoints({
  endpoints: (b) => ({
    getBookingStats: b.query({
      query: () => ({ url: "/api/bookings/stats" }),
      providesTags: ["Booking"],
    }),
    getAllBookings: b.query({
    ...
    cancelBooking: b.mutation({
      query: (id) => ({ url: `/api/bookings/${id}/cancel`, method: "PATCH" }),
...
const { useGetBookingStatsQuery, useGetAllBookingsQuery, useCancelBookingMutation } = bookingsApi;
...
export default function BookingsPage() {
  ...
  const { data, isLoading } = useGetAllBookingsQuery({ status, search, page, limit: 15 });
  ...
  const handleCancel = async () => {
    try {
      await cancelBooking(cancelItem._id).unwrap();
      setCancelItem(null);
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3.5 mb-6">
        <StatCard label="Total Bookings" value={stats?.total}               icon="ti-ticket"       color="bg-blue-50 text-blue-700"    sub={`${stats?.today ?? 0} today`}      loading={stLoading} />
        <StatCard label="Confirmed"      value={stats?.confirmed}           icon="ti-circle-check" color="bg-green-50 text-green-700"  sub={fmtMoney(stats?.confirmedRevenue)} loading={stLoading} />
        <StatCard label="Pending"        value={stats?.pending}             icon="ti-clock"        color="bg-amber-50 text-amber-700"  sub={fmtMoney(stats?.pendingRevenue)}   loading={stLoading} />
        <StatCard label="Cancelled"      value={stats?.cancelled}           icon="ti-ban"          color="bg-red-50 text-red-700"      sub={`${stats?.refundCount ?? 0} refunds`} loading={stLoading} />
      </div>
      ...
      <div className="bg-white border border-[#D0E6F7] rounded-2xl p-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
          <p className="text-[11px] font-semibold text-[#5A7089] uppercase tracking-[0.8px]">
            All bookings <span className="text-[#B0C4D8] font-normal">({total})</span>
          </p>
              ...
              {isLoading ? (
                Array.from({ length: 6 }).map((_, i) => (
                  <tr key={i}>{Array.from({ length: 8 }).map((__, j) => (
                    <td key={j} className="px-4 py-3"><div className="h-4 bg-[#EAF4FB] rounded animate-pulse w-16" /></td>
                  ))}</tr>
                ))
              ) : bookings.length === 0 ? (
                <tr><td colSpan={8} className="text-center py-10">
                  <i className="ti ti-ticket-off text-[32px] text-[#D0E6F7] block mb-2" />
                  <p className="text-[13px] text-[#7A90A4]">No bookings found</p>
                </td></tr>
              ) : bookings.map((b) => {
                ...
                return (
                  <tr key={b._id} className="hover:bg-[#F8FBFF] transition">
                    <Td><span className="font-bold text-[#1565C0] text-[12px]">{b.bookingReference}</span></Td>
                    <Td>
                      <div>
                        <p className="text-[12px] font-semibold text-[#0D1B2A] truncate max-w-[120px]">{b.passengerId?.name ?? "—"}</p>
                        <p className="text-[10px] text-[#B0C4D8] truncate max-w-[120px]">{b.passengerId?.email}</p>
                      </div>
                    </Td>
                    <Td>
                      <div>
                        <p className="text-[12px] font-semibold text-[#0D1B2A]">{b.flightId?.flightNumber ?? "—"}</p>
                        <p className="text-[10px] text-[#B0C4D8]">{b.flightId ? `${b.flightId.source} → ${b.flightId.destination}` : ""}</p>
                      </div>
                    </Td>
                    <Td>
                      <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full capitalize ${clCfg}`}>
                        {b.seatClass || "Economy"}
                      </span>
                    </Td>
                    <Td><span className="text-[12px] font-semibold text-[#0D1B2A]">{fmtMoney(b.totalAmount)}</span></Td>
                    <Td><span className="text-[12px] text-[#7A90A4]">{fmt(b.createdAt)}</span></Td>
                    <Td>
                      <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full capitalize ${stCfg.cls}`}>
                        {b.status}
                      </span>
                    </Td>
                    <Td>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between mt-4 pt-4 border-t border-[#EAF4FB]">
            <p className="text-[11px] text-[#B0C4D8]">Showing {bookings.length} of {total}</p>
            <div className="flex items-center gap-1.5">
      ...
      {/* View modal */}
      {viewItem && (
        <Modal title={`Booking — ${viewItem.bookingReference}`} onClose={() => setViewItem(null)}>
          <div className="space-y-0">
            {[
              ...
              ["Route",        viewItem.flightId ? `${viewItem.flightId.source} → ${viewItem.flightId.destination}` : "—"],
              ["Departure",    viewItem.flightId?.departureTime ? new Date(viewItem.flightId.departureTime).toLocaleString("en-IN") : "—"],
              ...
              <div key={label} className={`flex justify-between items-center py-2.5 ${i > 0 ? "border-t border-[#EAF4FB]" : ""}`}>
                <span className="text-[12px] font-medium text-[#7A90A4]">{label}</span>
                <span className="text-[12px] font-semibold text-[#0D1B2A] text-right max-w-[55%] truncate capitalize">{value ?? "—"}</span>
              </div>
            ))}
          </div>
          <button onClick={() => setViewItem(null)} className="mt-4 w-full h-10 bg-[#F0F7FF] border border-[#D0E6F7] text-[#1565C0] text-[13px] font-medium rounded-[10px] hover:bg-[#E1EFFE] transition cursor-pointer">
            Close
          </button>
        </Modal>
      )}
      ...
      {cancelItem && (
        <Modal title="Cancel booking" onClose={() => setCancelItem(null)}>
          <div className="text-center">
            ...
            <p className="text-[14px] font-semibold text-[#0D1B2A] mb-1">Cancel {cancelItem.bookingReference}?</p>
            <p className="text-[12px] text-[#7A90A4] mb-5">This will cancel the booking and trigger a refund if applicable.</p>
